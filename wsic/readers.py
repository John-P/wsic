...
class XarrayTileIterator(TileIterator):
    """An iterator which returns tiles from a reader which uses xarray.

    This iterator is used to read tiles from a reader which uses xarray
    (e.g. a reader which uses dask). It is used to read tiles from a
    reader in a way which is compatible with the TileIterator class.
    ...
    def __init__(
        ...
        self.queue: List[Tuple[Tuple[int, int], daskd.Future]] = []

    def fill_queue(self) -> None:
        """Enqueue futures to read tiles."""
        while len(self.queue) < self.num_workers:
            try:
                next_ji = self.remaining_reads.pop(0)
            except IndexError:
                break
            ...
            self.queue.append((next_ji, future))
            self.enqueued.add(next_ji)
...
class TIFFReader(Reader):
    ...
    def __init__(self, path: Path) -> None:
        ...
        # Normalise axes
        axes = "".join(self._dataset["0"].dims)
        if axes != "YXS":
            self._dataset["0"] = self._dataset["0"].transpose("Y", "X", "S")
    ...
    def thumbnail(self, shape: Tuple[int, ...], approx_ok: bool = False) -> np.ndarray:
        ...
        """
        da = self._dataset["0"]
        (_, _, downsample,) = self._find_thumbnail_downsample(
            shape,
            da.shape[:2],
            da.data.chunksize[:2],
        )
        thumbnail = (  # noqa: ECE001
            da.coarsen(X=downsample, boundary="trim")
            .mean()
